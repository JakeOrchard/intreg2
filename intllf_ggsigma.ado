...
version 13
		...
		*Point data
		...
		qui gen double `v' = exp((log($ML_y1 )-`delta')/`sigma') if $ML_y1 != .  ///
								& $ML_y2 != . & $ML_y1 == $ML_y2
		...
		*Interval data
			qui replace `zu' = ($ML_y2/exp(`delta'))^(1/`sigma') if $ML_y1 != . & $ML_y2 != . & ///
															$ML_y1 != $ML_y2
								
			qui replace `Fu' = gammap(`p',`zu') if $ML_y1 != . & $ML_y2 != . ///
								&  $ML_y1 != $ML_y2
								
			qui replace `zl' = ($ML_y1/exp(`delta'))^(1/`sigma') if $ML_y1 != . & $ML_y2 != .  ///
												&  $ML_y1 != $ML_y2
		...
		*Top coded data
			...
			qui replace `zu' = ($ML_y2/exp(`delta'))^(1/`sigma')	if $ML_y2 != . & $ML_y1 == .
