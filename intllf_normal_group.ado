...
for the normal distribution. Used with grouped data.
...
version 13
		...
		*Interval data
		 qui replace `Fu' = normal(($ML_y2-`mu')/`sigma') if $ML_y1 != . & ///
							$ML_y2 != . &  $ML_y1 != $ML_y2
							
		 qui replace `Fl' = normal(($ML_y1-`mu')/`sigma') if $ML_y1 != . & ///
							$ML_y2 != . &  $ML_y1 != $ML_y2
		...
		*Bottom coded data
		 ...
		 qui replace `Fl' = normal(($ML_y1-`mu')/`sigma') 
		 qui replace `lnf' = log(1-`Fl') if $ML_y1 != . & $ML_y2 == .
	
		
		*Top coded data
			
		 qui replace `Fu' = normal(($ML_y2-`mu')/`sigma') if $ML_y2 != . & $ML_y1 == .
